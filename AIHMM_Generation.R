...
#Daniele
pathToData <- "C:/Users/Daniele/Dropbox/pc_sharing/Thesis/R/Datasets/WindowsPhone_Daniele.csv" #

k <- 1
while(k <= 5)
{
  sink(paste("Experiment_2_First_Approach/test_baum_3_", k, "_interesting_sequence.txt"))
  loopOverSequenceSet(pathToData, k)
  k <- k + 1
  sink()
  print(paste("Done with k = ", k))
...
computeModelLogLikelihood <- function(sortedSequences, sequences, constrainedTrainedHMM, LogLikUnconst){
 ...
 #LogLikHmm requires list of sequences
  ...
  if(LogLikConst >= LogLikUnconst){
    ...
    print(paste("The Log-Likelihood is not no-worse than the unconstrained model and the process will skip this model", list(stop, constrainedTrainedHMM))) 
...
# #TO RUN. requires library hmm.discnp to compute the model performance
# computeModelLogLikelihood <- function(sortedSequences, sequences, symbols, constrainedTrainedHMM){
  # stop<-FALSE
  # library("hmm.discnp")
 ...
 # #LogLikHmm requires list of sequences
  ...
  # #build the unconstrained model with the same number of states
  # require(HMM)  
  # print("Build an unconstrained model with the same number of states")
  # HMMInit = initHMM(states, symbols)
  # unconstrainedHMM <- trainBaumWelch(HMMInit, as.vector(sequences))
  ...
  # LogLikUnconst = logLikHmm(sortedSequences, list(Rho=t(EmissMatrixUnconst), tpm = TransMatrixUnconst, ispd = StartProbsUnconst ) )
  # print("loglik of unconstrained model with same nr of states")
  # print(LogLikUnconst)
   # # the constrained model must have loglikelihood >= than the one of the unconstrained model 
  # if(LogLikConst >= LogLikUnconst){
  	# stop=TRUE
    ...
    # }
  # else{
    # print("The Log-Likelihood is not no-worse than the unconstrained model")     
  # }
 # if(stop==FALSE){
 	# print(list(stop, constrainedTrainedHMM))
  	# stop("Stopping process.")
  # }
  # return(list(stop, constrainedTrainedHMM, LogLikConst)) 
# }
